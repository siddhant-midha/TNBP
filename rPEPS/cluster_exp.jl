...
function main()
    ...
    # Option 2: Deduplicate clusters y using Set to automatically remove duplicates
    println("Collecting all unique clusters from all sites...")
    all_unique_clusters = Set{Cluster}()
    total_clusters_before_dedup = 0
    ...
    for site in 1:(N*T)
        ...
        total_clusters_before_dedup += length(clusters)
    ...
    for (sig, cluster_list) in clusters_by_signature
        if length(cluster_list) > 1
            manual_duplicates += length(cluster_list) - 1
        end
    end
